...
NetFunnel.TS_ACTION_ID		= "sample_01";				// Default TS Action id
...
NetFunnel.TS_USER_DATA_KEYS     = [{"key":"user_id", "type":"v"}];  // Input UserData Key & Type(c=cookie,v=variable)
//NetFunnel.TS_USER_DATA_KEYS	= [];					// Input UserData Key & Type(c=cookie,v=variable)
													// ex) [ {"key":<user_data_key>, "type":<c|v>}, ... ] 

// TODO 정환열 : IE 데모시 에러 발생하여 주석처리
// console.debug("=============================== " + JSON.stringify(NetFunnel.TS_USER_DATA_KEYS));
...
//Server Block
NetFunnel.TS_BLOCK_MSG			= "차단된 서비스입니다.";// Server Block시 팝업에 표시할 문구
...
NetFunnel.Util = {
    ...
    getTimeStr: function(g, D, K, b) {
        ...
        var x = false;
        var u = false;
        var F = D.match(/%[-]*[0-9]*[H|M|S]/g);
        for (var B = 0; B < F.length; B++) {
            var J = F[B];
            var H = J.charAt(J.length - 1);
            if (H == "H") {
                E = true
            }
            if (H == "M") {
...
NetFunnel.MProtect = function() {
    try {
        ...
        var a = parseInt(s.getItem("NFMPT.lastTime", true));
        if (isNaN(a) || a == null  || a == "") {
            a = 0
        }
        var r = parseInt(s.getItem("NFMPT.reqCnt", true));
        if (isNaN(r) || r == null  || r == "") {
            r = 0
        }
        var m = new Array();
        ...
        if (NetFunnel.gPop) {
            ...
            if (tTime) {
                if (NetFunnel.gLastData.real_time_left > NetFunnel.gControl.getConfig("showtime_limit")) {
                    var tformat = tTime.className;
                    var tformat_arr = tformat.split("^");
...
NetFunnel.Popup = function(content, top, left, winobj, waiting, obj_id, zindex) {
    ...
    if (!tObj || NetFunnel.SkinUtil.prevID != NetFunnel.gSkinId) {
        ...
        if (tObj_ct_busy) {
            ...
            var busyHeight = parseInt(tObj_ct_busy.style.height);
            if (!isNaN(busyWidth)) {
    ...
    if (!popmask) {
        ...
        popmask.style.zIndex = "" + eval(this._mZindex + 1);
...
NetFunnel.Popup.prototype.show = function() {
    ...
    this._mObj.style.zIndex = "" + eval(this._mZindex + 2);
...
NetFunnel.TsClient.prototype.setComplete = function(h, f, d, e) {
    ...
    j.setItemStorageOnly("NFMPT.reqCnt", "0");
    if (NetFunnel.TS_BYPASS == true) {
        this.fireEvent(null , this, "onSuccess", {
            rtype: this._mReqType,
            code: NetFunnel.kSuccess,
...
function NetFunnel_setComplete(e, f) {
    ...
    if (typeof f != "undefined" && f.constructor == Object) {
        ...
        if (a == null  || a == "") {
            ...
            if (!d) {
                var g = new NetFunnel.Storage(2);
                g.removeItem(NetFunnel.gControl.mConfig.cookie_id);
                NetFunnel.gControl._sendError(NetFunnel.RTYPE_SET_COMPLETE, NetFunnel.kErrorSystem);
                return false
            }
            h = b.getValue("ip");
